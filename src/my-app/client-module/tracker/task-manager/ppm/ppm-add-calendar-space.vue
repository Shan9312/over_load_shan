...
<template>
  <div class="add-ppm-calendar m-b-80">
            ...
            <div class="calender-col-item"
              ...
              <div class="col-item"
                ...
                <span
                  ...
                  :key="index">
                  <b></b>
                  <!--<i class="el-icon-arrow-down el-icon&#45;&#45;right drop-down"-->
                     <!--@click.stop="activeBox = keep.taskId + '-' + index"></i>-->
                  <div v-if="activeBox === keep.taskId + '-' + item"
                       class="spaceBox" @click.stop="">
                       <p class="header-title">相关空间</p>
                       ...
                       <div class="btns">
                         <el-button @click="editTaskSpaceList" >确认</el-button>
                       </div>
                  </div>
                </span>
              </div>
    ...
    <div class="fixed-control">
      ...
      <el-button  type="primary" @click="confirmData">保存并完成</el-button>
    </div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    * */
    confirmData() {
      this.$message.success('ppm计划保存成功');
      this.$router.push({
        name: 'ppmtask-planAdd',
      });
    },
    /**
    *点击空间确认按钮 编辑当个格子
    ...
    * */
    editTaskSpaceList() {
      ...
      const obj = {
        id: '',
        pid: this.showBox.taskId,
        weekNo: this.weekNo,
        valid: 1,
        spaceIds: this.multipleSelection.map(son => son.spaceId),
      };
      this.showBox.gridList.forEach((grid) => {
        if (Number(grid.weekNo) === this.weekNo) {
          obj.id = grid.taskId;
        }
      });
      arr.push(obj);
    ...
    //   } }).then((res) => {
    //     this.handleSpaceList(JSON.parse(JSON.stringify(res)));
    //   });
    // },
    /**
    *把空间列表数据 格式 拍平
    ...
    * */
    ...
    //       if (item.spaceSonList && item.spaceSonList.length) {
    //         item.spaceSonList.forEach((child) => {
    //           this.tableData3.push(child);
    //           this.handleSpaceList(child.spaceSonList);
    //         });
    //       }
    //     });
    //   }
          ...
          this.systemList.forEach((item) => {
            item.upkeepList.forEach((keep) => {
              ...
              keep.gridList.forEach((grid) => {
                ...
                // grid.spaceList = JSON.parse(JSON.stringify(keep.spaceList));
              });
     ...
     *
     * */
    chooseWeek(item, weekNo) {
      console.log(item, weekNo);
      this.tableData3 = JSON.parse(JSON.stringify(item.spaceList));
      if (weekNo <= this.nowWeek) {
        this.$message.error('不可操作当前周之前的计划');
        return false;
      }
      this.showBox = item;
      this.weekNo = weekNo;
      this.activeBox = `${item.taskId}-${weekNo}`;
      const arr = item.gridNum;
      const pos = arr.indexOf(weekNo);
      if (pos < 0) {
        this.activeBox = -1;
      }
      item.gridList.forEach((child) => {
        if (weekNo === Number(child.weekNo)) {
          this.tableData3.forEach((obj, index) => {
            if (child.spaceList.find(ele => ele.spaceId === obj.spaceId)) {
              this.$nextTick(() => {
                this.$refs.multipleTable[0].toggleRowSelection(this.tableData3[index]);
              });
            }
          });
        }
  ...
  .add-ppm-calendar{
    ...
    .col-item{
      ...
      .spaceBox{
        ...
        box-shadow: 0px 0px 14px 0px rgba(114, 133, 152, 0.3);
        z-index: 1000;
        .header-title{
          font-size: 16px;
          color: #14112d;
