 ...
 */
<template>
<div class="p-b-80 daily-task-tpl-add">
  ...
  <div class=" edit-content">
    <el-form label-width="100px" :model="taskForm"  ref="taskForm" :rules="taskFormRules">
      ...
      <el-row v-if="!isStatic" class="m-t-20">
        <!--任务周期-->
        <el-col :span="8">
  ...
  methods: {
    ...
    submit() {
      if (this.isStatic) {
          ...
          if (valid) {
            if (this.taskForm.virtualBeaconId !== '') {
              this.$http.post(this.api.taskApi.staticAdd, {
                ...
                .then((res) => {
                  if (res) {
                    this.$message.success('新增固定岗任务成功');
        ...
        // 这里是新建路径岗任务
        this.$refs.taskForm.validate((valid) => {
          console.log(this.taskForm.locationSelection);
          if (valid) {
            if (this.chooseBeaconList.length > 0) {
              this.$http.post(this.api.taskApi.routeAdd, {
                ...
                .then((res) => {
                  if (res) {
                    this.$message.success('新增巡视岗任务成功');
                    this.$router.push({ name: 'dailyTaskTplList' });
                  }
                });
            } else {
