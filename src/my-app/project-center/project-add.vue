...
<template>
  ...
  <div class="project-add edit-content">
    <el-form :model="projectForm"
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="客户公司" prop="companyId">
            ...
            @change="getSonCompanyById(projectForm.companyId)" placeholder="请选择">
              <el-option
        ...
        <el-col :span="16">
          <el-form-item label="合作公司">
            <el-select v-model="projectForm.companySubIds"
          ...
          <span v-if="!projectForm.companyId" class="node-title">注* 请先选择客户公司</span>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
      ...
      <el-row>
        <el-col :span="24" class="upload-item-icon">
          ...
          <!--prop icon 无法检测到上传组件值的变化 用input 绑定值做替代 并隐藏-->
          <el-input v-model="projectForm.background" v-show="false"></el-input>
      ...
      <div class="related">
        ...
        <div class="item" v-for="(item,index) in projectForm.projectCompanyList" :key="item.date">
          <i class="el-icon-minus" @click="deleteItem('projectCompanyList',index)"></i>
          <el-row>
            <el-col :span="8">
              <el-form-item label="客户名称"
                            :prop="'projectCompanyList.'+ index + '.companyId'"
            ...
            <el-col :span="8">
              <el-form-item label="角色类型"
                            :prop="'projectCompanyList.'+ index + '.relationCompany'"
                            :rules="projectRules.related"
              >
      ...
      <div class="related">
        ...
        <div class="item" v-for="(item,index) in projectForm.projectLinkmanList" :key="item.date">
          <i class="el-icon-minus" @click="deleteItem('projectLinkmanList',index)"></i>
          <el-row>
            <el-col :span="8">
              <el-form-item label="联络人名称"
                            :prop="'projectLinkmanList.'+ index + '.linkmanId'"
                            :rules="projectRules.related"
              >
                <el-select v-model="item.linkmanId"
                           filterable
                >
            ...
            <el-col :span="8">
              <el-form-item label="角色类型"
                            :prop="'projectLinkmanList.'+ index + '.relationLinkman'"
                            :rules="projectRules.related"
              >
      ...
      <div class="related">
        ...
        <div class="item" v-for="(item,index) in projectForm.projectCertList" :key="item.date">
          <i class="el-icon-minus" @click="deleteItem('projectCertList',index)"></i>
          <el-row>
            <el-col :span="8">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      customerList: [], // 合作公司 选项
      companySonArr: [], // 合作子公司选项
      // blueTooth
      blueTooth: [
        { value: '1', label: '旧蓝牙' },
        { value: '2', label: '新蓝牙' },
  ...
  methods: {
    ...
    *
    * */
    getSonCompanyById(id) {
      this.$http.post(`${this.api.projectApi.getSonList}?companyId=${id}`)
        .then((res) => {
    ...
    addItem(type) {
      switch (type) {
        case 1:
          // 新增客户
          this.projectForm.projectCompanyList.push({ companyId: '', relationCompany: '' });
          break;
        case 2:
          // 新增联络人
          this.projectForm.projectLinkmanList.push({ linkmanId: '', relationLinkman: '' });
          break;
        case 3:
          if (this.projectForm.projectCertList === null) {
            this.projectForm.projectCertList = [];
          }
          this.projectForm.projectCertList.push({ remark: '', pic1: '', category: 1 });
    ...
    deleteItem(val, index) {
      ...
      if (val === 'projectCertList') {
        this.uploadProps.pop();
      }
    },
    /**
     * 获取 经度，纬度
     ...
     * */
    showInfo(e) {
      this.map.clearOverlays(); // 每次先清除上一次的经纬度
      this.pointObj = {
        lng: e.point.lng,
        lat: e.point.lat,
      };
    ...
    editProject() {
      ...
      const linkman = [];
      form.projectLinkmanList.forEach((item) => {
        const newItem = {
          linkmanId: item.linkmanId,
          relationLinkman: item.relationLinkman,
        };
        linkman.push(newItem);
      });

      const customerMan = [];
      form.projectCompanyList.forEach((item) => {
        const newItem = {
          companyId: item.companyId,
          relationCompany: item.relationCompany,
        };
      ...
      this.$refs.projectFormRef.validate((valid) => {
        if (valid) {
          this.$http.post(this.api.projectApi.insertProject, obj)
            .then((res) => {
              if (res) {
