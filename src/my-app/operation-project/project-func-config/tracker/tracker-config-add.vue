    ...
    <div class="header m-b-30">
      <div v-content class="p-b-20">
        <el-row>
          <el-col :span="8" class="border-right">
            <el-form :model="form" label-width="100px" ref="form" :rules="rules">
              ...
              <el-form-item label="功能图标" class="p-r-30" prop="iconId">
                ...
                <span class="img-item cursor" @click="openIconDialog">
          ...
          <el-col :span="8" class="p-l-30 border-right">
            <div class="task-type">
              <div class="title">
                功能类型
              </div>
              <div class="content m-t-30">
                <span class="item m-r-15 m-b-15 cursor"
                      :class="{'active': activeBasicType.id === item.id}"
                      v-for="item in result"
                      :key="item.id"
                      @click="chooseTaskBasicType(item)"
                >{{item.nameCn}}</span>
              </div>
            </div>
          </el-col>
          <el-col :span="8" class="p-l-30" v-if="activeBasicType.id === '2'">
            <div class="task-type">
...
export default {
  ...
  data() {
    return {
      ...
      permissionId: this.$route.params.permissionId, // 一级权限id
  ...
  methods: {
     ...
     * */
    getPermission() {
      this.$http.get(this.api.operationApi.getPermissionTree, { params: { id: this.permissionId } })
        .then((res) => {
          this.result = res.children;
          this.result = this.result.filter(v => v.isBindBasicType === '1');
    ...
    getPermissionProject() {
      this.$http.get(
        ...
        .then((res) => {
          ...
          const arr = this.projectPermission.map(v => v.id); // 找出改项目权限id
          this.result = this.result.filter(v => arr.indexOf(v.id) !== -1); // 和二级权限取交集
          if (this.result.length > 0) {
            this.chooseTaskBasicType(this.result[0]); // 设置默认显示第一个
          } else {
    ...
    chooseTaskBasicType(item) {
      if (this.tableData.length > 0) {
        this.$confirm('此操作将清空表格数据, 是否继续?', '提示', {
          ...
          this.activeBasicType = item;
          this.tableData = [];
        }).catch(() => {});
      } else {
        this.activeBasicType = item;
      }
      this.getTableHeader(item.children);
    },
    /**
     * 显示动态表头
     *
     ...
     * */
    confirmEditTag() {
      if (this.tagForm.name !== '' && this.tagForm.name.length < 9) {
        const obj = {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          const obj = {
            ...
            secondLvPermissionId: this.activeBasicType.id,
          ...
          this.$http.post(this.api.operationApi.addBizTaskType, obj)
            .then((res) => {
              if (res) {
                ...
                this.$deleteTag();
                this.$router.push({ name: 'operationDeploy', params: { id: this.projectId } });
              }
            });
        }
      });
