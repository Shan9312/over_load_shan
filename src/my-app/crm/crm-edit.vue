  ...
  <div class="crm-edit tables " v-edit>
      <el-form
        ...
        <!--基本信息-->
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="关系等级" prop="relationLv">
                ...
                >
                  <el-select v-model="item.id"
                    ...
                    <el-option v-for="(item,index) in customerList" :key="index"
                               :label="item.name"
                               :disabled="customerList.indexOf(item) === -1"
...
export default {
  ...
  methods: {
     ...
     *  */
    getMsg() {
      this.$http.post(this.api.crmApi.getCrmDetail, {
        id: this.id,
      }).then((res) => {
        const obj = res;
        this.$setTitle(res.name);
        this.crmForm = obj;
        this.$cookie.set('title', this.crmForm.name); // 设置标签栏显示的详情名称
      });
    },
    /**
     *查询客户列表
    ...
    /**
     *查询项目列表
     * pageNum=1&pageSize=9999&searchName默认空值
     * */
    getProjectList() {
    ...
    editItem() {
      this.$refs.crmFormRef.validate((valid) => {
        if (valid) {
          this.$http.post(this.api.crmApi.editCrm, this.crmForm)
            .then((res) => {
              if (res) {
                this.$message({
                  message: '修改成功',
