<template>
  <div class="personnel-add">
      ...
      <el-form :model="personnelForm"
        ...
        <el-row>
          ...
          <el-col :span="24" class="upload-item-icon">
            <i class="el-icon-plus" @click="uploadImg(uploadProps[0].name)"></i>
            <el-form-item label="人员头像">
              <upload
                :ref="uploadProps[0].name"
                @getUrl="uploadLogo"
                :name="uploadProps[0].name"
                :list="logoList"
              />
            </el-form-item>
          </el-col>
        ...
        <el-row class="basic-panel">
          <el-col :span="8">
            <el-form-item label="登录名" prop="loginName">
              <el-input v-model="personnelForm.loginName"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="密码" prop="psd"  class="inline-form-choice">
              <el-input v-model="personnelForm.psd" ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
              ...
              <el-col :span="8">
                ...
                >
                  <el-select v-model="item.projectId"
                             filterable
                             @change="selectChildProject(item.projectId, index)"
                             placeholder="请选项目名称">
    ...
    <!--选择职能弹窗-->
    <func-dialog :visible="visible" @changeClose="changeClose"
    :deptTree="deptTree" @getPositionObj="getPositionObj" :isChoosePosition="true"></func-dialog>
    <!--选择职能弹窗-->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    * */
    getPositionObj(data) {
      this.personnelForm.accountQueries[this.indexProjetc].positionName = data.positionObj.name;
      this.personnelForm.accountQueries[this.indexProjetc].positionId = data.positionObj.id;
    ...
    * 根据项目 查询 子公司
    * */
    selectChildProject(id, index) {
      // 改变选中项目时  清空 所属公司 和部门职能
    ...
    submitForm() {
      this.$refs.personnelForm.validate((valid) => {
        if (valid) {
          this.personnelForm.password = sha1(this.personnelForm.psd);
