...
*/
<template>
    <div class="company-edit edit-content ">
      <panelContentHeader>
      </panelContentHeader>
          ...
          <el-row class="basic-panel">
            ...
            <el-col :span="8">
              <el-form-item label="客户编码">
                <el-input  v-model="companyForm.companyCode" disabled></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
          ...
          <el-row>
            <el-col :span="16">
              <el-form-item label="详细地址" >
                <el-input  v-model="companyForm.address" placeholder="请输入联络地址"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
          ...
          <div class="related-info related">
            ...
            <div class="item"
              ...
              <i class="el-icon-minus"
                 @click="deleteItem('companySubUpsertQueries',index,item)"></i>
              <el-row>
                <el-col :span="8">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        address: [
          { required: true, message: '请输入公司联络地址', trigger: 'blur' },
        ],
        billAccountNo: [
  ...
  methods: {
    ...
    addItem(type) {
      switch (type) {
        ...
        case 2:
          // 新增项目
          this.companyForm.projectList.push({ relationCompany: '', id: '' });
          break;
        case 3:
          // 新增相关客户
    ...
    deleteItem(val, index, item) {
      if (val === 'companySubUpsertQueries') {
      // 如果是删除的相关客户 valid 值变为-1 作隐藏
        if (item.id) {
          item.valid = -1;
        } else {
    ...
    submitForm() {
      this.$refs.companyForm.validate((valid) => {
        if (valid) {
          this.$http.post(this.api.companyApi.editCompany, this.companyForm)
            .then((res) => {
              if (res) {
                this.$message({
                  message: '修改成功',
    ...
    getCompanyDetails() {
      this.$http.post(this.api.companyApi.getCompanyDetail, { id: this.id })
        .then((res) => {
          ...
          this.$set(this.companyForm, 'companySubUpsertQueries', res.companySubList);
          this.$set(this.companyForm, 'zoneId', res.zone.id);
